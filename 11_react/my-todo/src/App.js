...
function App() {
  ...
  // id 값은 렌더링되는 정보가 아니기 때문에 ref 사용 단순히 새로운 항목을 만들 때 참조되는 값임
  const nextId = useRef(4);

  // props로 전달해야 할 함수를 만들 때는 useCallback()을 사용해본다!
  // useCallback() 미사용 시 컴포넌트가 재렌더링 될 때마다 새롭게 정의됨
  // => props로 넘겨지는 값이 바뀌므로 자식 컴포넌트가 재 랜더링
  ...
  const handleRemove = useCallback((id) => {
    ...
    // copyTodos.splice(targetIndex, 1); //인덱스를 넣어주고 1개를 삭제하겠다
    // setTodos(copyTodos);
...
// HTML 웹 스토리지란?
// 웹스토리지를 사용하면 웹 앱이 사용자의 브라우저 내에 로컬로 데이터를 저장할 수 있음
// 웹 스토리지는 도메인 당 사용 가능
...
// window.sessionStorage - 한 세션에 대한 데이터를 저장 (브로우저 탭을 닫으면 데이터가 손실됨)
// npm install uuid
